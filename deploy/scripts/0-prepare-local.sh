...
echo ""
echo "===================================="
echo "DONE. Next step (本项目共存安全路径 / this project's coexistence-safe path):"
echo "  bash $SCRIPT_DIR/syn-sync-flink-image.sh   # 分发镜像+compose+.env（不含 jar）"
echo "  bash $SCRIPT_DIR/2-up-all.sh               # 只重建变更的 flink 容器"
echo "  bash $SCRIPT_DIR/syn-upload-m1.sh --jar-only  # 投放本项目 jar（正确文件名）"
echo ""
echo "注意 / NOTE: 请勿运行 1-sync-to-nodes.sh —— 它是旧 FA-iForest 骨架，会把 jar 以旧名"
echo "  ($JOB_JAR_NAME) 覆盖到集群共享 jars/，冲掉旧项目产物（违反共存）。本项目的 jar 由"
echo "  syn-upload-m1.sh 以 SYN_JOB_JAR_NAME 单独投放。Do NOT run 1-sync (overwrites the old jar)."
echo "===================================="
