...
checks=(
    ...
    "com/leejean/m1/M1Job.class|AlignEventTime|事件时间对齐修复（M2 增补 方案 A：RoundAssembler 定时器偏移 30s）"
    "com/leejean/m2/M2Job.class|AlignEventTime|事件时间对齐修复（M2 增补 方案 A，M2 联合作业侧）"
    "com/leejean/m2/M2Job.class|checkpoint-tolerable-failures|checkpoint 容忍失败次数（akka.framesize 越限诊断用，默认 0）"
    "org/deeplearning4j/nn/multilayer/MultiLayerNetwork.class|MultiLayerNetwork|DL4J 1.0.0-M2.1 LSTM 自编码器引擎（Java 11 运行时，M3 阶段 DEV-D1）"
