...
## 2026-09-18 缺陷与修复（已解除暂停）：M2 曾只纳入每条轮流的一半
...
修复已按 M2 补充件 Option A 实施：在 `RoundAssembler` 之后把事件时间重赋为轮的标称时间（`M1Job`
与 `M2Job` 各一处），并附 `PmcodTimestampOffsetTest`（钉住 δ=0/10s/30s 下的 60/50/30）与
`M1M2TimestampAlignmentTest`（作业级断言 δ=0、监测标签落在分钟边界、迟到为零、几何恢复到 60）。
**本手册已可重跑**；重跑即为该修复的验证，第 5 节的逐设备等值核验预期在 ±1% 内通过。
