...
> **磁盘与集群状态前置条件（2026-09-19 实测修订）**：第一次盘点发现两件事。其一，三台节点上
> `zookeeper`、`kafka-1`、`jobmanager` 三个核心容器**都不存在**（两台 worker 上一个容器都没有），
> 集群处于「已拆除」而非「已关机」状态，因此下面第 2.4 步是**创建**容器，且 Kafka 会以空卷启动、
> 所有 topic 需要用 `syn-create-topics.sh` 重建。其二，磁盘占用的主体是历次 Kafka 容器遗留的
> **孤儿匿名卷**（master 182 个、worker-1 60 个、worker-2 61 个，在用仅 2 个），不是容器可写层。
> 清理步骤、安全边界与不可撤销动作见 `docs/cluster_disk_cleanup_zh.md`（2.0 版）。必须先完成清理
> 并复核三台节点各留出 20 GB 以上余量，才能进入第 3 节。
